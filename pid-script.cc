...
static void PktInQueueEQ(Ptr<OutputStreamWrapper> stream1, unsigned int before, unsigned int now ){*stream1->GetStream () << Simulator::Now ().GetSeconds () << "\t" << now << std::endl;}

//trace-sink for QueueDisc IQueue (Queuelength, Qdisc)
static void PktInQueueIQ(Ptr<OutputStreamWrapper> stream2, unsigned int before, unsigned int now ){*stream2->GetStream () << Simulator::Now ().GetSeconds () << "\t" << now << std::endl;}

//dynamic trace connection
static void TraceFunc(Ptr<QueueDisc> qdiscPtr, Ptr<Queue<Packet>> extqPtr)
{
  AsciiTraceHelper asciiTraceHelper;
  Ptr<OutputStreamWrapper> stream0 = asciiTraceHelper.CreateFileStream ("/media/sf_SharedFolder/nsplots/Jplots/cwndp.dat");
  Ptr<OutputStreamWrapper> stream1 = asciiTraceHelper.CreateFileStream ("/media/sf_SharedFolder/nsplots/Jplots/qsizep.dat");
  Ptr<OutputStreamWrapper> stream2 = asciiTraceHelper.CreateFileStream ("/media/sf_SharedFolder/nsplots/Jplots/qdsizep.dat");
...
int main ()
{
    ...
    double sstart = cstart + 150;        //secs   //start step-change in desired queue-length ; 
    ...
    double cstop  = cstart + 350;        //secs   //clientApp stop time; runtime + cstart
    ...
    uint32_t QsizeRefPktsEQ = 175;      //linearization equilibrium pont of queue-length
    uint32_t QsizeRefPktsDQ = 375;      //desired queue-length; step-change = QsizeRefPktsDQ - QsizeRefPktsEQ
...
NS_LOG_INFO ("\nSETTING DEFAULT PARAMETERS\n"); 
    ...
    Config::SetDefault ("ns3::TcpSocketBase::Sack", BooleanValue (isSack));                 //default is true // also change to m_sackEnabled {false}; in /src/internet/model/tcp-tx-buffer.h //PATCH1
    Config::SetDefault ("ns3::TcpSocket::DelAckCount", UintegerValue (0));                  //default is 2 packets //DelayAckTimeout < MinRTO is same proportion as defaults
    ...
    Config::SetDefault ("ns3::TcpSocket::InitialCwnd", UintegerValue (1));                  //default is 1 packet size  //Equilibrium Point
    ...
    Config::SetDefault ("ns3::PidQueueDisc::QueueSizeReferenceDQ", QueueSizeValue (QueueSize (QueueSizeUnit::PACKETS, QsizeRefPktsDQ)));
      ...
      //PID (see equation 15)
      double N, T, u0, a, b, c, d, e;
      T = 0.00625; N = 0; u0 = 0.0085;
      ...
      std::cout << "\tDiscretePIDparameters: " << "\t" << Simulator::Now ().GetSeconds () << "\t" << u0  << "\t\t" << a << "\t" << b << "\t" << c << "\t" << d << "\t" << e << "\n" << std::endl;
      //uk = (m_a * u_{k-1})  + (m_b * u_{k-2})  +  ( m_c * e_{k} )  +  ( m_d * e_{k-1} )  +  ( m_e * e_{k-2} );  
      //m_dropProb0 = (m_a * m_dropProb1)  + (m_b * m_dropProb2)  +  ( m_c * m_ErrQsize0 )  +  ( m_d * m_ErrQsize1 )  +  ( m_e * m_ErrQsize2 );  (see equation 15)
      //m_dropProb2 = m_dropProb1; m_dropProb1 = m_dropProb0;
    ...
    Config::SetDefault ("ns3::PidQueueDisc::N", UintegerValue (N));                         //derivative term filter cooefficient
    ...
    Config::SetDefault ("ns3::PidQueueDisc::Sstep", TimeValue (Seconds (sstart)));          //start step-change in desired queue-length


    //delete previous-simulation output-data files
    remove("/media/sf_SharedFolder/nsplots/Jplots/cwndp.dat");
    remove("/media/sf_SharedFolder/nsplots/Jplots/qsizep.dat");
    remove("/media/sf_SharedFolder/nsplots/Jplots/qdsizep.dat");
    remove("/media/sf_SharedFolder/nsplots/Jplots/dprobp.dat");
...
NS_LOG_INFO ("CREATING DUMBELL TOPOLOGY\n"); 
    ...
    // Install Stack
        ...
        CON_qDiscsLeftRouterR = tchBottleneck.Install (devLeftRouterR.Get(0));                     //the TrafficControlHelper Install command requires Ptr< NetDevice as argument   
         //CON_qDiscsRightRouterR = tchBottleneck.Install (devRightRouterR.Get(0));                //calls PidQDisc constructor, which has an event auto-scheduled. So do not uncomment unless you plan to use, becoz if 
                                                                                                   //... if you delete/remove afterward, the constructor-scheduled-event will attempt to access a null pointer giving error
    ...
    // Using NetAnim animator        
        AnimationInterface anim ("/media/sf_SharedFolder/nsplots/Jplots/allTraces/animV.xml");	
        BoundingBox (1, 1, 100, 100, nLeafL, nLeafR, ncLeft, ncRouter, ncRight);	
    ...
    // Additional pcap tracing
        ...
        helper.EnablePcapAll ("/media/sf_SharedFolder/nsplots/Jplots/allTraces/pcapT", false);
...
//simulation phase
    ...
    //monitor->SerializeToXmlFile("/media/sf_SharedFolder/nsplots/Jplots/allTraces/flowM.xml", true, true);         //comment this if above "additional pcap tracing" is commented
